...
def get_company_lat_lng():
    ...
    tianditu_map_ak = request.env['ir.config_parameter'].sudo().get_param('map_world_app_key')
    ...
    ds_json = json.dumps(addr_key_word, ensure_ascii=False)
    # 请求参数
    params = {
        "ds": ds_json,  # 地址关键字
        "tk": tianditu_map_ak  # 天地图APPKey
    }
...
class EstatePropertyController(Home):
    ...
    def get_baidu_property_map_markers(self, property_id, **kwargs):
        rtn_response = get_response(property_id)
        return rtn_response

    @http.route(['/estate/tianditu_map/get_markers'], type='http', auth="user", website=True, sitemap=True)
    def get_tianditu_property_map_markers(self, property_id, **kwargs):
        rtn_response = get_response(property_id)
        return rtn_response
    ...
    @http.route(['/estate/tianditu_map/get_all_properties'], type='http', auth="user", website=True, sitemap=True)
    def get_tianditu_properties_all(self, **kwargs):
    ...
    @http.route(['/estate/tianditu_map/get_company_loc'], type='http', auth="user", website=True, sitemap=True)
    def get_tianditu_company_marker(self, **kwargs):
        location = get_company_lat_lng()
        company_marker = [{
            'latitude': location["latitude"],
            'longitude': location["longitude"],
    ...
    @http.route(['/estate/tianditu_map/get_ak'], type='http', auth="public", website=True, sitemap=True)
    def get_tianditu_map_ak(self, **post):
        # 从系统参数中获取百度地图AK
        tianditu_map_ak = request.env['ir.config_parameter'].sudo().get_param('map_world_web_key')
        _logger.info(f"map_world_web_key={tianditu_map_ak}")
        return request.make_response(
            json.dumps({'ak': tianditu_map_ak}),  # 将AK转换为JSON
            headers=[('Content-Type', 'application/json')]
        )
