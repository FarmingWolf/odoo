...
class ParkVehicleLogContract(models.Model):
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _name = 'park.vehicle.log.contract'
    ...
    vehicle_id = fields.Many2one('park.vehicle', 'Vehicle', required=True, check_company=True)
    cost_subtype_id = fields.Many2one('park.service.type', 'Type', help='Cost type purchased with this cost', domain=[('category', '=', 'contract')])
    ...
    service_ids = fields.Many2many('park.service.type', string="Included Services")
    ...
    def write(self, vals):
        ...
        if vals.get('expiration_date') or vals.get('user_id'):
            self.activity_reschedule(['parking.mail_act_park_contract_to_renew'], date_deadline=vals.get('expiration_date'), new_user_id=vals.get('user_id'))
        return res
    ...
    def scheduler_manage_contract_expiration(self):
        ...
        reminder_activity_type = self.env.ref('parking.mail_act_park_contract_to_renew', raise_if_not_found=False) or self.env['mail.activity.type']
        ...
        for contract in nearly_expired_contracts:
            contract.activity_schedule(
                'parking.mail_act_park_contract_to_renew', contract.expiration_date,
