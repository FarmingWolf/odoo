...
class ParkVehicleLogServices(models.Model):
    _name = 'park.vehicle.log.services'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _rec_name = 'service_type_id'
    _description = 'Services for vehicles'

    active = fields.Boolean(default=True)
    vehicle_id = fields.Many2one('park.vehicle', 'Vehicle', required=True)
    manager_id = fields.Many2one('res.users', 'Parking Manager', related='vehicle_id.manager_id', store=True)
    ...
    service_type_id = fields.Many2one(
        'park.service.type', 'Service Type', required=True,
        default=lambda self: self.env.ref('parking.type_service_service_7', raise_if_not_found=False),
    )
