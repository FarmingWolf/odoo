...
export class ApprovalComment extends Component {
    // static template = "contract_expense.ApprovalComment";
    ...
    async onClickApproval() {
        ...
        try {
            const svr_action = "contract_expense." + this.state.actionType + "_svr_action"
...
ApprovalComment.template = "contract_expense.ApprovalComment";
ApprovalComment.components = { Dialog };
ApprovalComment.props = {
    ...standardWidgetProps,
};

registry.category("actions").add("contract_expense.approval_comment_dialog", ApprovalComment);
