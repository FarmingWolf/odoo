...
class EstateLeaseContractPropertyFeeElectricityMaintenance(models.Model):
    _name = "estate.lease.contract.property.fee.electricity.maintenance"
    _description = "资产租赁合同电力维护费"
    _order = "date_received"
    _inherit = ['mail.thread', 'mail.activity.mixin']

    name = fields.Char(string="电力维护费实缴详情", default="电力维护费实缴详情")
    ...
    electricity_maintenance_receivable = fields.Float(default=0.0, string="本期应收(元)", tracking=True, help="账单金额")
    electricity_maintenance_received = fields.Float(default=0.0, string="本期实收(元)", tracking=True)

    date_received = fields.Date(string="实收日期", default=lambda self: fields.Date.context_today(self), tracking=True)
    electricity_maintenance_received_sum = fields.Float(default=0.0, string="累计实收(元)", readonly=True,
                                                        compute="_compute_received", store=True, compute_sudo=True,
                                                        help="合同保存后，系统自动计算累计值")
    electricity_maintenance_arrears = fields.Float(string="本期欠缴(元)", readonly=True, store=True,
                                                   compute="_compute_received", compute_sudo=True)
    electricity_maintenance_arrears_sum = fields.Float(string="累计欠缴(元)", readonly=True, store=True,
                                                       compute="_compute_received", compute_sudo=True,
                                                       help="合同保存后，系统自动计算累计值")
    ...
    electricity_maintenance_receipt_editable = fields.Boolean(
        string="发票信息可编辑", compute="_compute_electricity_maintenance_receipt_editable")
    ...
    def _compute_electricity_maintenance_receipt_editable(self):
        for record in self:
            record.electricity_maintenance_receipt_editable = \
    ...
    def _compute_receipt_type_editable(self):
        ...
        for record in self:
            if not is_editable:
                ...
                if record.electricity_maintenance_receipt:
                    record.receipt_type_editable = False
                else:
    ...
    def _onchange_electricity_receivable(self):
        self.electricity_maintenance_arrears = self.electricity_maintenance_receivable - \
                                               self.electricity_maintenance_received
    ...
    @api.depends("electricity_maintenance_received", "electricity_maintenance_receivable", "date_received")
    def _compute_received(self):
        for record in self:
            domain = [('contract_rental_plan_rel_id', '=', record.contract_rental_plan_rel_id.id)]
            rcds = self.env["estate.lease.contract.property.fee.electricity.maintenance"].search(domain)
            received_sum = 0.0
            arrears_sum = 0.0

            for rcd in rcds:
                received_sum += rcd.electricity_maintenance_received
                if rcd.electricity_maintenance_received_sum != received_sum:
                    rcd.electricity_maintenance_received_sum = received_sum

                arrears = rcd.electricity_maintenance_receivable - rcd.electricity_maintenance_received
                arrears_sum += arrears
                if rcd.electricity_maintenance_arrears != arrears:
                    rcd.electricity_maintenance_arrears = arrears

                if rcd.electricity_maintenance_arrears_sum != arrears_sum:
                    rcd.electricity_maintenance_arrears_sum = arrears_sum

    def action_apply_electricity_maintenance_invoice(self):
        for record in self:
            if record.electricity_maintenance_received < 0.01:
            ...
            if record.receipt_status != 'applied':
                ...
                record._set_default_electricity_maintenance_receipt_evidence()

            if not record.receipt_type:
                record.receipt_type = 'zp'

    @api.onchange("electricity_maintenance_receipt")
    def _onchange_electricity_maintenance_receipt(self):
        if self.electricity_maintenance_receipt:
            self.receipt_done_by_uid = self.env.user.id
            self.receipt_done_date = date.today()
            self._set_default_electricity_maintenance_receipt_evidence()

    @api.depends("electricity_maintenance_receipt", "electricity_maintenance_receipt_evidence")
    def _compute_receipt_status(self):
        for record in self:
            if record.electricity_maintenance_receipt and record.electricity_maintenance_receipt_evidence:
    ...
    def _set_default_electricity_maintenance_receipt_evidence(self):
        if not self.electricity_maintenance_receipt_evidence:
            tmp_str = []
            if self.renter_id:
            ...
            self.electricity_maintenance_receipt_evidence = tmp_str
